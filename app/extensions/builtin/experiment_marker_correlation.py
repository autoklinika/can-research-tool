...
class _EventWindow:
    ...
    first_change: dict[
        CandidateKey,
        tuple[_FrameEvidence, _FrameEvidence, int, int, int],
    ...
    def observe(self, source_row: int, frame: CanFrame) -> None:
        ...
        for byte_index in range(common):
            ...
            self.post_last[(key, byte_index)] = (evidence, after_byte)
            changed_mask = before_byte ^ after_byte
            if not changed_mask:
                continue
...
class ExperimentMarkerCorrelationProvider:
    ...
    def run(self, context: AnalysisContext) -> Artifact:
        ...
        events = tuple(
            _MarkerEvent.from_payload(item) for item in parameters["marker_events"]
        )
        ...
        if not target_events:
            raise ValueError(
                "experiment marker correlation requires at least one target marker event"
            )
        ...
        outside = sorted(
            {
                event.session_id
                for event in events
                if event.session_id not in allowed_sessions
            }
        )
        if outside:
            raise ValueError(
                f"marker events reference sessions outside comparison set: {outside}"
            )

        sources = {
            session_id: context.project.session(session_id)
        ...
        context.progress.report(
            0,
            total_work,
            "correlating marker windows with immutable CAN frames",
        )
        observations: dict[
            CandidateKey,
            dict[str, list[dict[str, Any]]],
        ] = defaultdict(lambda: {"target": [], "control": []})
        ...
        for session_id in comparison.session_ids:
            ...
            session_events = sorted(
                ...
                key=lambda item: (item.timestamp_ns, item.marker_id),
            )
            windows = [
                _EventWindow(
                    marker=event,
            ...
            for source_row, frame in enumerate(source.frames.iter_frames()):
                ...
                while (
                    ...
                    and windows[next_window].pre_start_ns <= timestamp
                ):
                    active.append(windows[next_window])
                    next_window += 1
                active = [
                    window for window in active if timestamp <= window.post_end_ns
                ]
            ...
            context.progress.report(
                processed,
                total_work,
                f"analysed markers in {source.name}",
            )
        ...
        ranked = _rank_candidates(
            ...
            maximum_evidence_events=parameters[
        ...
        artifact_sources = tuple(
            ArtifactSource(
                ...
                source_reference={
                    ...
                    "marker_event_count": len(
                        events_by_session.get(source.id, ())
                    ),
        ...
        context.progress.report(
            total_work,
            total_work,
            "saved experiment marker correlation",
        )
        return artifact
    ...
    *,
    ...
    for candidate, grouped in observations.items():
        ...
        if not target_changed:
            continue
        target_unchanged = [item for item in target if not item["changed"]]
        control_changed = [item for item in control if item["changed"]]
        control_unchanged = [item for item in control if not item["changed"]]

        target_eligible = len(target)
        control_eligible = len(control)
        target_support = (
            len(target_changed) / target_eligible if target_eligible else 0.0
        )
        target_coverage = (
            target_eligible / target_event_count if target_event_count else 0.0
        )
        control_rate = (
            len(control_changed) / control_eligible if control_eligible else 0.0
        )
        specificity = 1.0 - control_rate if control_eligible else 1.0
        directions = Counter(
            f"{item['before_state']}->{item['after_state']}"
            for item in target_changed
        )
        ...
        delays = [
            int(item["delay_ns"])
            for item in target_changed
            if item["delay_ns"] is not None
        ]
        message_key, byte_index, bit_index = candidate

        # Preserve both positive and negative evidence. Changed target/control
        # events are listed first, followed by qualifying no-change events.
        # This makes claims such as "0/5 controls" auditable without rescanning
        # the source sessions. The list remains explicitly bounded.
        ordered_evidence = (
            target_changed
            + target_unchanged
            + control_changed
        ...
        evidence_rows = [
            ...
            for item in ordered_evidence[:maximum_evidence_events]
        ]
        evidence_event_count = len(ordered_evidence)
        key_payload = _message_key_payload(message_key)
        ranked.append(
            {
                **key_payload,
                ...
                "bit_index": bit_index,
                "candidate_key": (
                    f"{key_payload['message_key']}:B{byte_index}.{bit_index}"
                ),
                "score": _round(score),
                "target": {
                ...
                "timing": {
                    ...
                    "mean_delay_ns": (
                        _round(sum(delays) / len(delays)) if delays else None
                    ),
                    "median_delay_ns": (
                        _round(float(median(delays))) if delays else None
                    ),
                },
                "evidence": evidence_rows,
                "evidence_event_count": evidence_event_count,
                "evidence_changed_event_count": (
                    len(target_changed) + len(control_changed)
                ),
                "evidence_truncated": (
...
def _comparison_input(context: AnalysisContext):
    if context.comparison is None:
        raise ValueError(
            "experiment marker correlation requires comparison context"
        )
    inputs = [
        item for item in context.inputs if item.kind == "comparison_set"
    ]
    if len(inputs) != 1:
        raise ValueError(
            "experiment marker correlation requires exactly one comparison_set input"
        )
...
def _parameters(values: Mapping[str, Any]) -> dict[str, Any]:
    ...
    pre_window_ms = _bounded_float(
        values.get("pre_window_ms", _DEFAULT_PRE_WINDOW_MS),
        "pre_window_ms",
        0.001,
        60_000.0,
    )
    post_window_ms = _bounded_float(
        values.get("post_window_ms", _DEFAULT_POST_WINDOW_MS),
        "post_window_ms",
        0.001,
        60_000.0,
    )
    maximum_ranked_candidates = _bounded_int(
        values.get(
            ...
            _DEFAULT_MAXIMUM_RANKED_CANDIDATES,
        ),
        "maximum_ranked_candidates",
        1,
    ...
    return {
        ...
        "control_marker": _selector_payload(
            values.get("control_marker"),
            allow_empty=True,
        ),
...
def _selector_payload(
    ...
    *,
    ...
    return {
        ...
        "label": str(
            value.get("label", value.get("name", selector))
        ),
    }
